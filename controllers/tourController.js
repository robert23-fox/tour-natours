...
exports.resizeTourImages = catchAsync(async(req, res, next) => {
  ...
  // Don't use here forEach method, but instead use map because you need to await all the promises and push them in the req.body.images
...
exports.getToursWithin = catchAsync(async (req, res, next) => {
  ...
  const [lat, lng] = latlng.split(","); //here are [] because splite creates an array

  const radius = unit === "mi" ? distance / 3963.2 : distance / 6378.1; // the radius = the distance / Radius of the Earth
...
exports.getDistances = catchAsync(async (req, res, next) => {
  ...
  const distances = await Tour.aggregate([
    ...
    {
      $project: {
        //$project defines what fields we want to keep
        distance: 1,
        name: 1,
      },
    },
